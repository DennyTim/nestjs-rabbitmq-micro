...
export class BuyCourseSagaStateStarted extends BuyCourseSagaState {
  public async pay(): Promise<{ paymentLink: string; user: UserEntity }> {
    ...
    if (!course) {
      throw new Error("Current course isn't existed");
    }
  ...
  public checkPayment(): Promise<{ user: UserEntity; status: PaymentStatus }> {
    throw new Error("No way to check the payment that hasn't yet started");
  }
...
export class BuyCourseSagaStateWaitingForPayment extends BuyCourseSagaState {
  public pay(): Promise<{ paymentLink: string; user: UserEntity }> {
    throw new Error("No ability to create payment links while the payment is in progress");
  }
  ...
  public async checkPayment(): Promise<{ user: UserEntity; status: PaymentStatus }> {
    ...
    return { user: this.saga.user, status: "progressing" };
  }

  public cancel(): Promise<{ user: UserEntity }> {
    throw new Error("No way to cancel payment while the payment is in progress");
  }
}

export class BuyCourseSagaStatePurchased extends BuyCourseSagaState {
  public pay(): Promise<{ paymentLink: string; user: UserEntity }> {
    throw new Error("No way to pay for the already bought course");
  }

  public async checkPayment(): Promise<{ user: UserEntity; status: PaymentStatus }> {
    throw new Error("No way to check payment by bought course");
  }

  public cancel(): Promise<{ user: UserEntity }> {
    throw new Error("No way to cancel the already bought course");
  }
}
...
export class BuyCourseSagaStateCanceled extends BuyCourseSagaState {
  ...
  public async checkPayment(): Promise<{ user: UserEntity; status: PaymentStatus }> {
    throw new Error("No way to check payment by canceled course");
  }

  public cancel(): Promise<{ user: UserEntity }> {
    throw new Error("No way to cancel the already canceled course");
  }
}
